...
class KGGDS:
    ...
    def _createGraphProjection(self) -> None:
        self._calculateSimilarity()
    ...
    def createLaidenCommunity(self):
        G, res = self._createGraphProjection()
        gds =  neoGDS()
        with gds:
            gds.leiden.write(
            G,
    ...
    def _calculateSimilarityResume(self):
        Q_Delete_Rel = "MATCH ()-[s:MATCHED_SKILL]-() DELETE s"
        Q_Create_Rel="""
                MATCH(r:Resume)-[k:KNOWS]->(rs:ResumeSkill)
    ...
    def _createNewResumeProjection(self):
        self._calculateSimilarityResume()
    ...
    def streamLeidenOnResume(self,email:str)->pd.DataFrame:
        self._createNewResumeProjection()
        print(email)
        Q_Leiden = """
...
if __name__ == "__main__":
    ...
    data = KGGDS().streamLeidenOnResume('srijita.de@gmail.com')
    print(data)
